...
#ifndef CLIENT_DLL
static float FoFBowSignedSpread( float flMagnitude )
{
	return random->RandomInt( 0, 100 ) > 50 ?
		-flMagnitude : flMagnitude;
}

static void FoFFireBowShootEvent( CFoF_Player *pOwner )
{
...
static void FoFFireBowArrow( CFoF_Player *pOwner )
{
	...
	if ( nAccuracy == 3 )
	{
		vecForward.x += FoFBowSignedSpread( 0.06f );
		vecForward.y += FoFBowSignedSpread( 0.06f );
	}
	else if ( nAccuracy < 1 || nAccuracy > 2 )
	{
		vecForward.x += FoFBowSignedSpread( 0.15f );
		vecForward.y += FoFBowSignedSpread( 0.15f );
	}
	...
	const int iDamage = static_cast< int >( RemapValClamped(
		flSight, 0.5f, 1.0f, 25.0f, 45.0f ) );
	CBowarrowBolt *pArrow = CBowarrowBolt::BoltCreate(
		vecOrigin, angArrow, iDamage, false, pOwner );
	if ( pArrow )
	{
...
CWeaponBowarrow::CWeaponBowarrow()
{
...
bool CWeaponBowarrow::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	return BaseClass::Holster( pSwitchingTo );
}

bool CWeaponBowarrow::Reload()
{
	return DefaultReload( GetMaxClip1(), GetMaxClip2(), ACT_VM_RELOAD );
}

void CWeaponBowarrow::PrimaryAttack()
{
	if ( m_iClip1 <= 0 )
	{
		if ( !m_bFireOnEmpty )
	...
	if ( m_nCurrentAmmo == 0 )
	{
		FoFFireBowShootEvent( pOwner );
		FoFFireBowArrow( pOwner );
	}
#endif
...
void CWeaponBowarrow::SecondaryAttack()
{
}

void CWeaponBowarrow::ItemPostFrame()
{
	CFoF_Player *pOwner = ToFoFPlayer( GetOwner() );
	if ( !pOwner || !pOwner->IsAlive() )
		return;
...
bool CWeaponBowarrow::SendWeaponAnim( int iActivity )
{
...
#ifndef CLIENT_DLL
void CWeaponBowarrow::Operator_HandleAnimEvent(
	animevent_t *pEvent,
...
IMPLEMENT_NETWORKCLASS_ALIASED( WeaponBowarrow, DT_WeaponBowarrow )

BEGIN_NETWORK_TABLE( CWeaponBowarrow, DT_WeaponBowarrow )
#ifdef CLIENT_DLL
...
#ifdef CLIENT_DLL
BEGIN_PREDICTION_DATA( CWeaponBowarrow )
...
FOF_LINK_WEAPON_CLASS( weapon_bow, CWeaponBowarrow )
#else
LINK_ENTITY_TO_CLASS( weapon_bow, CWeaponBowarrow );
