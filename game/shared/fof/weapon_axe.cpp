...
static const Vector s_FoFAxeHullMins( -16.0f, -16.0f, -16.0f );
static const Vector s_FoFAxeHullMaxs( 16.0f, 16.0f, 16.0f );

#ifndef CLIENT_DLL
static CHL2MP_Player *FoFStartAxeLagCompensation( CBasePlayer *pOwner )
{
...
static void FoFFinishAxeLagCompensation( CHL2MP_Player *pLagPlayer )
{
	if ( pLagPlayer )
		lagcompensation->FinishLagCompensation( pLagPlayer );
}

static void FoFApplyAxeDamage(
	CWeaponAxe *pWeapon,
	CBasePlayer *pOwner,
	trace_t &traceHit )
{
	...
	{
		FoFRecordAccuracyHit( pFoFOwner, 40.0f );
	}
	...
	CTakeDamageInfo info( pOwner, pOwner, 40.0f, DMG_CLUB );
...
static void FoFRefineAxeHullTrace(
	trace_t &traceHit,
	...
	CBasePlayer *pOwner )
{
	...
	{
		s_FoFAxeHullMins.Base(), s_FoFAxeHullMaxs.Base()
	};
	...
	trace_t &traceHit )
{
	...
	UTIL_TraceHull(
		vecStart, vecEnd, s_FoFAxeHullMins, s_FoFAxeHullMaxs,
	...
	if ( vecToTarget.Dot( vecForward ) < 0.70721f )
		...
		FoFRefineAxeHullTrace( traceHit, vecStart, pOwner );
}

static Activity FoFAxeAttackActivity(
	CWeaponAxe *pWeapon,
	CBasePlayer *pOwner,
	trace_t &traceHit )
{
...
#else
	FoFApplyAxeDamage( pWeapon, pOwner, traceHit );
	pWeapon->WeaponSound(
...
void CWeaponAxe::FoFPrimarySwing()
{
	...
	CHL2MP_Player *pLagPlayer = FoFStartAxeLagCompensation( pOwner );
#endif

	trace_t traceHit;
	FoFTraceAxe( pOwner, GetRange(), traceHit );
	const Activity attackActivity =
		FoFAxeAttackActivity( this, pOwner, traceHit );
	if ( traceHit.fraction != 1.0f )
		ImpactEffect( traceHit );
#ifndef CLIENT_DLL
	FoFFinishAxeLagCompensation( pLagPlayer );
#endif
	FoFFinishAxeSwing( this, pOwner, attackActivity );
}
...
void CWeaponAxe::ItemPostFrame()
{
		...
		flNow > m_flDelayedFire && m_bShotDelayed )
	{
		...
		CAxeBolt *pProjectile = CAxeBolt::BoltCreate(
			vecOrigin, angProjectile, 45, pOwner );
		if ( pProjectile )
			pProjectile->SetAbsVelocity( vecForward * 950.0f );
#endif
...
bool CWeaponAxe::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	m_bShotDelayed = false;
...
float CWeaponAxe::GetFireRate()
{
	return 0.75f;
}

float CWeaponAxe::GetDamageForActivity( Activity )
{
	return 40.0f;
}
...
FOF_LINK_WEAPON_CLASS( weapon_axe, CWeaponAxe )
#else
