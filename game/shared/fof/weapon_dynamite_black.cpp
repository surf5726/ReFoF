...
#endif
CWeaponDynamiteBlack::CWeaponDynamiteBlack()
{
...
Activity CWeaponDynamiteBlack::PrimaryPullbackActivity() const
{
	return ACT_VM_PULLBACK_HIGH;
}

float CWeaponDynamiteBlack::PrimaryReleaseDelay() const
{
	return 1.2f;
}

float CWeaponDynamiteBlack::PrimaryCountdownDuration() const
{
	return 7.0f;
}

bool CWeaponDynamiteBlack::ShouldThrowOnHolster() const
{
	return !IsBlackDynamite();
}

bool CWeaponDynamiteBlack::ShouldReleasePrimaryThrow(
	CFoF_Player *pOwner ) const
{
	return pOwner && !( pOwner->m_nButtons & IN_ATTACK );
}

bool CWeaponDynamiteBlack::ShouldDecrementAmmoOnThrow() const
{
	return true;
}

void CWeaponDynamiteBlack::PreparePrimaryPullback( CFoF_Player *pOwner )
{
	NOTE_UNUSED( pOwner );
}

float CWeaponDynamiteBlack::PrimaryDrawbackPredictionLeadTime() const
{
	return gpGlobals->interval_per_tick;
}

float CWeaponDynamiteBlack::ThrowEventPredictionLeadTime() const
{
	return gpGlobals->interval_per_tick;
}

float CWeaponDynamiteBlack::RedrawFallbackTime()
{
	return 0.0f;
}

void CWeaponDynamiteBlack::Precache()
{
	BaseClass::Precache();
#ifndef CLIENT_DLL
...
#ifdef CLIENT_DLL
int CWeaponDynamiteBlack::DrawModel( int flags )
{
...
bool CWeaponDynamiteBlack::Deploy()
{
	m_bRedraw = false;
	m_fDrawbackFinished = false;
	m_flCountdown = 0.0f;
...
bool CWeaponDynamiteBlack::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	CFoF_Player *pOwner = FoFDynamiteOwner( GetOwner() );
...
#ifdef CLIENT_DLL
void CWeaponDynamiteBlack::SetDormant( bool bDormant )
{
	m_bRedraw = false;
	m_fDrawbackFinished = false;
	m_flReleaseTime = 0.0f;
...
void CWeaponDynamiteBlack::PrimaryAttack()
{
	CFoF_Player *pOwner = FoFDynamiteOwner( GetOwner() );
...
void CWeaponDynamiteBlack::SecondaryAttack()
{
	if ( !IsBlackDynamite() )
		return;
...
void CWeaponDynamiteBlack::ItemPostFrame()
{
	CFoF_Player *pOwner = FoFDynamiteOwner( GetOwner() );
#ifdef CLIENT_DLL
...
bool CWeaponDynamiteBlack::Reload()
{
	if ( !HasPrimaryAmmo() )
		return false;
...
void CWeaponDynamiteBlack::BeginThrowAnimation(
...
void CWeaponDynamiteBlack::CompleteThrow( CFoF_Player *pOwner, int nEvent )
{
#ifndef CLIENT_DLL
...
void CWeaponDynamiteBlack::ThrowPresentation( CFoF_Player *pOwner )
{
#ifndef CLIENT_DLL
	StopSound( "Weapon_Dynamite.Burn" );
#endif
...
void CWeaponDynamiteBlack::DecrementAmmo( CBaseCombatCharacter *pOwner )
{
	if ( !pOwner )
		return;
...
#ifdef CLIENT_DLL
bool CWeaponDynamiteBlack::OnFireEvent( C_BaseViewModel *pViewModel,
...
#else
void CWeaponDynamiteBlack::Operator_HandleAnimEvent(
...
bool CWeaponDynamiteBlack::IsFoFPrimedDeathDynamite() const
{
	return IsBlackDynamite() && m_flCountdown > gpGlobals->curtime;
}

void CWeaponDynamiteBlack::SpawnFoFDeathDynamite(
...
void CWeaponDynamiteBlack::SpawnDynamite( CFoF_Player *pOwner, int nEvent )
{
	if ( !pOwner )
		return;
...
#endif
IMPLEMENT_NETWORKCLASS_ALIASED( WeaponDynamiteBlack, DT_WeaponDynamiteBlack )

BEGIN_NETWORK_TABLE( CWeaponDynamiteBlack, DT_WeaponDynamiteBlack )
#ifdef CLIENT_DLL
...
#ifdef CLIENT_DLL
BEGIN_PREDICTION_DATA( CWeaponDynamiteBlack )
...
FOF_LINK_WEAPON_CLASS( weapon_dynamite_black, CWeaponDynamiteBlack )
#else
LINK_ENTITY_TO_CLASS( weapon_dynamite_black, CWeaponDynamiteBlack );
PRECACHE_WEAPON_REGISTER( weapon_dynamite_black );
#endif
