	...
	float flDamage )
{
	...
	{
		FoFRecordAccuracyHit( pFoFOwner, flDamage );
	}
...
void CWeaponFists::PrimaryAttack()
{
	BeginFistSwing( false );
}

void CWeaponFists::SecondaryAttack()
{
	BeginFistSwing( true );
}

void CWeaponFists::BeginFistSwing(
	bool bSecondary )
{
...
void CWeaponFists::ResolveFistHit()
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if ( !pOwner || !pOwner->IsAlive() )
		return;
...
#ifndef CLIENT_DLL
	CFoF_Player *pFoFOwner = ToFoFPlayer( pOwner );
	FoFRecordAccuracyShots( pFoFOwner, 1 );
	CHL2MP_Player *pLagPlayer =
...
void CWeaponFists::ItemPostFrame()
{
...
float CWeaponFists::GetRange()
{
	return 55.0f;
}

float CWeaponFists::GetFireRate()
{
	return 0.5f;
}

float CWeaponFists::GetDamageForActivity(
	Activity activity )
{
	if ( IsGhostFists() )
	{
...
FOF_LINK_WEAPON_CLASS( weapon_fists, CWeaponFists )
#else
